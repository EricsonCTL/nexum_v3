...
# Skill NEXUM V9.2 — Cadastro, pesquisa e Tabela Zero
...
Emitir `schema: "nexum-skill-json"` e `schema_version: "9.2"`.
...
8. Se houver ambiente NEXUM, executar somente prévia e comparar: contagem, preço, área, tipologia, agrupador, status, planos e marcas brutas. Sem prévia, declarar: `Validado contra o contrato V9.2; prévia NEXUM não executada.`
...
## Reconstrução de preços por unidade — regra V9.2
...
## Conclusão da Zero e integridade do arquivo

- Usar a mesma identidade de unidade e agrupador em Zero, março e setembro. Não inferir o significado de um sufixo de código sem comprovação ou mudar agrupadores entre fotografias.
- A simulação autoriza estimar preços; os códigos e áreas precisam de evidência ou reconstrução estrutural sustentada. Pesquisar plantas, memoriais, relação de quadras e lotes, books e tabelas complementares.
- Se a estrutura completa não puder ser reconstruída, registrar quantidade individualizada e faltante, entregar REVISAO e informar “reconstrução parcial”. Não chamar zero registros de Tabela Zero concluída.
- Preservar sempre a raiz canônica abaixo; não substituir tabela por tabela_zero, schema por nexum-v9.2, status_processamento por status, nem operacao por RECONSTRUCAO_TABELA_ZERO.
- Toda lacuna identificada deve constar em pendencias como objeto campo/observacao/status. ID de nova tabela pode ser null e será atribuído pelo NEXUM; isso, isoladamente, não é falha na pesquisa.
- Modelos ilustrativos não substituem pesquisa cadastral: emitir todos os dados adicionais comprovados nos campos canônicos já definidos, incluindo características, comercial e entrega.

## Modelo completo de estrutura JSON

Exemplo sintaticamente válido e deliberadamente parcial, com uma unidade fictícia. Substituir identidade, valores e fontes pelos dados pesquisados. Nunca copiar este exemplo como cadastro real. Para declarar reconstrução completa, preencher todas as unidades sustentadas e validar a contagem contra o total.
...
```json
{
    ...
    "unidades": [
      {
        ...
        "origem": {
          ...
          "formula": "100000 * 0.5",
          "fontes": [],
          "observacao": "Exemplo fictício sem fontes reais; não importar."
        }
      }
    ]
  },
  "resumo_comercial": {
    "total_estrutural": 2,
    "unidades_individualizadas": 1,
    "unidades_faltantes": 1,
    "reconstrucao_completa": false,
    "vgv_simulado_parcial": 50000
  },
  "comparacao": {
    "previousTableId": null,
    "novas": [],
  ...
  "auditoria": [
    {
      "evento": "EXEMPLO_ESTRUTURAL",
      "descricao": "Dados fictícios; dia 01 representa precisão de mês.",
      "origem": "SIMULADO"
    }
  ],
  "validacoes": [
    {
      "regra": "completude_estrutural",
      "resultado": "PENDENTE",
      "observacao": "Uma das duas unidades fictícias individualizada."
    }
  ],
  "pendencias": [
    {
      "campo": "tabela.unidades",
      "observacao": "Faltam estrutura completa, fontes reais e vínculo confirmado.",
      "status": "PENDENTE"
    }
  ]
}
