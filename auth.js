...
function enhanceNexumNavigationV2() {
  ...
  portfolio.innerHTML = `<button class="nav-enterprise-trigger ${isEnterpriseContext ? 'active' : ''}" type="button" aria-expanded="false">Empreendimentos <span>⌄</span></button><section class="enterprise-nav-panel" hidden><div class="enterprise-nav-search"><input type="search" placeholder="Buscar empreendimento" aria-label="Buscar empreendimento"><a class="button teal" href="${pageUrl('empreendimento.html')}?novo=1">+ Novo</a></div><div class="enterprise-nav-results" role="listbox"><p>Carregando empreendimentos…</p></div></section>`;
  ...
  search.innerHTML = '<span aria-hidden="true">⌕</span><input type="search" placeholder="Buscar empreendimentos, códigos, IDs" aria-label="Busca global de empreendimentos"><i aria-hidden="true"></i><section class="nav-global-results" hidden role="listbox"></section>';
